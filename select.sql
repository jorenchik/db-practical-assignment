...
WITH dl3 AS (
	SELECT dl1.darbinieka_id,
		...
		(SELECT TOP 1 stundas_likme FROM (
			...
			WHERE dl2.darba_sakums <= @PERIODA_SAKUMS AND dl2.darba_beigas >= @PERIODA_BEIGAS 
...
DECLARE @PERIODA_SAKUMS datetime = '2023-06-06 00:00:00', @PERIODA_BEIGAS datetime = '2023-06-07 00:00:00';
SELECT * FROM (
...
DECLARE @PERIODA_SAKUMS datetime = '2023-06-06 00:00:00', @PERIODA_BEIGAS datetime = '2023-07-08 00:00:00';
SELECT  DATEPART(YEAR, m.izpildijuma_laiks) gads, DATEPART(MONTH, m.izpildijuma_laiks) mēnesis, DATEPART(DAY, m.izpildijuma_laiks) diena, sum(summa) summa
	...
	WHERE m.izpildijuma_laiks > @PERIODA_SAKUMS AND @PERIODA_BEIGAS > m.izpildijuma_laiks
...
DECLARE  @PERIODA_SAKUMS datetime = '2023-06-04 00:00:00', @PERIODA_BEIGAS datetime = '2023-06-08 20:19:00';
WITH dv1 AS (
...
EXCEPT SELECT  t3.telpas_numurs, t3.prezentesanas_iespeja, ietilpigums, (SELECT darba_vietu_skaits FROM dv1 WHERE dv1.telpas_numurs = t3.telpas_numurs) darba_vietu_skaits, t3.nodalas_nosaukums
	 ...
	 WHERE t3.darbinieku_izmantosanas_iespeja = 1 AND NOT (tip.beigu_laiks < @PERIODA_SAKUMS OR @PERIODA_BEIGAS < tip.sakuma_laiks);
