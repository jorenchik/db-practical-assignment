...
DECLARE @PERIODA_SAKUMS datetime = '2023-06-01 00:00:00', @PERIODA_BEIGAS datetime = '2023-06-30 00:00:00';
...
DECLARE @DARBINIEKA_ID datetime = 1031;
WITH vp1 AS (
...
DECLARE @PERIODA_SAKUMS datetime = '2023-06-07 00:00:00', @PERIODA_BEIGAS datetime = '2023-06-08 00:00:00';
SELECT * FROM (
...
WHERE dsb.registrets > @PERIODA_SAKUMS AND @PERIODA_BEIGAS > dsb.registrets AND dsb.veida_kods != 7
) dsb1 WHERE dsb1.beigas is not null and dsb1.veida_kods in (1,3)
 
-- 7. Atskaite par dienas peļņu no telpas īres, ienesīgako telpu nedēļā (no rēķiniem).

SELECT  DATEPART(YEAR, m.izpildijuma_laiks) gads, DATEPART(DAY, m.izpildijuma_laiks) mēnesis, DATEPART(DAY, m.izpildijuma_laiks) diena, sum(summa) summa FROM maksajumi m
	...
	GROUP BY DATEPART(YEAR, m.izpildijuma_laiks), DATEPART(DAY, m.izpildijuma_laiks), DATEPART(DAY, m.izpildijuma_laiks)
...
DECLARE @NODALA nvarchar(50) = 'Datorikas nodaļa', @PERIODA_SAKUMS datetime = '2023-06-04 00:00:00', @PERIODA_BEIGAS datetime = '2023-06-06 20:19:00';
;
DECLARE 
